...
//TITLE: LipSync_Gaming_Firmware
//AUTHOR: MakersMakingChange
//VERSION: 3 (08 Jun 2022)
...
#define OUTPUT_BUTTON_ONE           (int)1             // Press and release button one or button X1(Left USB)/View(Right USB) in XAC
#define OUTPUT_BUTTON_TWO           (int)2             // Press and release button two or button X2(Left USB)/Menu(Right USB) in XAC
#define OUTPUT_BUTTON_THREE         (int)3             // Press and release button three or button LS(Left USB)/RS(Right USB) in XAC
#define OUTPUT_BUTTON_FOUR          (int)4             // Press and release button four or button LB(Left USB)/RB(Right USB) in XAC
#define OUTPUT_BUTTON_FIVE          (int)5             // Press and release button five or button A(Left USB)/X(Right USB) in XAC
#define OUTPUT_BUTTON_SIX           (int)6             // Press and release button six button B(Left USB)/Y(Right USB) in XAC
...
#define ACTION_VLONG_PUFF   OUTPUT_BUTTON_FIVE       // Default: Press and release button five or button A(Left USB)/X(Right USB) in XAC
#define ACTION_VLONG_SIP    OUTPUT_BUTTON_SIX        // Default: Press and release button six button B(Left USB)/Y(Right USB) in XAC
...
#define JOYSTICK_DELAY 5                          // Current joystick delay [ms]
...
#define LIPSYNC_MODEL (byte)2                     // LipSync Gaming
...
_functionList setDeadzoneFunction =               {"DZ,1", 1, &setDeadzone};
_functionList getButtonModeFunction =             {"BM,0", 0, &getButtonMode};
_functionList setButtonModeFunction =             {"BM,1", 1, &setButtonMode};
...
_functionList apiFunction[27] =
{
  ...
  getDeadzoneFunction,
  setDeadzoneFunction,
  getButtonModeFunction,
  setButtonModeFunction,
...
void setup() {
  ...
  initializeJoystick();                                    // Initialize Joystick
...
void loop() {
  ...
  joystickHandler();                                       // Read the joystick values and output joystick movements.
...
//
// Description: This function initializes the joystick.
//
// Parameters :  void
//
// Return     : void
...
void initializeJoystick(void)
{
  ...
  g_lastButtonState[0] = 0;
  g_lastButtonState[1] = 0;
  g_lastButtonState[2] = 0;
  g_lastButtonState[3] = 0;
  g_lastButtonState[4] = 0;
  g_lastButtonState[5] = 0;
...
//
// Description: This function handles the joystick movements based on the FSR values
//
// Parameters :  void
//
// Return     : void
...
void joystickHandler(void)
{
  ...
  // Measure FSR joystick and determine whether to output joystick commands
  outputJoystick = readJoystick(xJoy, yJoy, xHigh, xLow, yHigh, yLow);
  ...
  {
    sendDebugRawData(xJoy, yJoy, sipAndPuffRawHandler(), xHigh, xLow, yHigh, yLow);
...
//              joystick movements. Outputs true if mouse should be moved.
//
...
//
// Return     : outputJoystick : bool : This variable is used to indicate if mouse values should be outputted or skipped.
//*********************************//
...
// Test if radial position is outside circular deadband
 ...
 bool joystickLifted = (xHigh < JOYSTICK_LIFT_THRESOLD)
  ...
  if( outsideDeadzone && (aboveDelta || joystickLifted) )
  {
    ...
    g_xHighMapped=getMappedFSRValue(xHigh, g_deadzone, g_xHighNeutral, JS_MAPPED_IN_DEADZONE, JS_MAPPED_IN_MAX, g_xHighEquation);
    g_xLowMapped=getMappedFSRValue(xLow, g_deadzone, g_xLowNeutral, JS_MAPPED_IN_DEADZONE, JS_MAPPED_IN_MAX, g_xLowEquation);
    g_yHighMapped=getMappedFSRValue(yHigh, g_deadzone, g_yHighNeutral, JS_MAPPED_IN_DEADZONE, JS_MAPPED_IN_MAX, g_yHighEquation);
    g_yLowMapped=getMappedFSRValue(yLow, g_deadzone, g_yLowNeutral, JS_MAPPED_IN_DEADZONE, JS_MAPPED_IN_MAX, g_yLowEquation);
...
// Function   : setXYAxis
//
// Description: This function moves the joystick based on the two input coordinates.
//
// Parameters : x : int : the input x joystick value.
//              y : int : the input y joystick value.
//
// Return     : void
//********************//
...
//              Output format: "LOG:1:0,0,0,g_xHighNeutral,g_xLowNeutral,g_yHighNeutral,g_yLowNeutral"
//              Output format: "LOG:2:0,0,0,g_xHighMax,g_xLowMax,g_yHighMax,g_yLowMax"
//
// Parameters :  void
//
// Return     : void
...
//              Output format: "LOG:3:xJoy,yJoy,Action,xUp,xDown,yUp,yDown"
//
// Parameters :  x : int : The Joystick x movement.
//               y : int : The Joystick y movement.
//               action : int : The Joystick button actions.
...
//***GET DEADZONE FUNCTION***//
// Function   : getDeadzone
//
// Description: This function retrieves the current deadzone.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
...
// Return     : void
//*********************************//
int getDeadzone(bool responseEnabled, bool apiEnabled) {
  int deadzone = JOYSTICK_DEADBAND;
  if (API_ENABLED)
  {
    EEPROM.get(EEPROM_deadzoneValue, deadzone);
    if (deadzone < JOYSTICK_DEADBAND || deadzone> JOYSTICK_MAX_DEADZONE)
    {
      deadzone = JOYSTICK_DEADBAND;
      EEPROM.put(EEPROM_deadzoneValue, deadzone);
      delay(EEPROM_WRITE_DELAY);
    }
  }
...
//***GET DEADZONE API FUNCTION***//
// Function   : getDeadzone
//
// Description: This function is redefinition of main getDeadzone function to match the types of API function arguments.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
...
//
// Return     : void
void getDeadzone(bool responseEnabled, bool apiEnabled, int* optionalArray)
{
  if (optionalArray[0] == 0)
  {
    getDeadzone(responseEnabled, apiEnabled);
  }
}
...
//
// Description: This function sets the deadzone.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
//                                   Manual response is sent if it's set to false.
//               inputDeadzone : int : The new the deadzone.
//
// Return     : void
//*********************************//
void setDeadzone(bool responseEnabled, bool apiEnabled, int inputDeadzone)
{
  bool isValidFactor = true;
  if (inputDeadzone >= JOYSTICK_DEADBAND && inputDeadzone <= JOYSTICK_MAX_DEADZONE) // Check if inputDeadzone is within range
  {
    // Valid inputDeadzone
    g_deadzone = inputDeadzone;
    EEPROM.put(EEPROM_deadzoneValue, inputDeadzone);
    delay(EEPROM_WRITE_DELAY);

    if(!API_ENABLED){ g_deadzone = JOYSTICK_DEADBAND; }
    isValidFactor = true;
  }
  else
  {
    //Invalid inputDeadzone
    EEPROM.get(EEPROM_deadzoneValue, g_deadzone);
    delay(10); 
    isValidFactor = false;
  }
...
//***SET BUTTON MODE API FUNCTION***//
// Function   : setDeadzone
//
// Description: This function is redefinition of main setDeadzone function to match the types of API function arguments.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
//                                   Manual response is sent if it's set to false.
//               inputDeadzone : int* : The array of one element which contains the new deadzone.
//
// Return     : void
void setDeadzone(bool responseEnabled, bool apiEnabled, int* inputDeadzone)
{
  setDeadzone(responseEnabled, apiEnabled, inputDeadzone[0]);
}
...
//
// Description: This function retrieves the current button mode.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
...
// Return     : void
//*********************************//
int getButtonMode(bool responseEnabled, bool apiEnabled) {
  int buttonMode = BUTTON_MODE;
  if (API_ENABLED)
  {
...
//***GET BUTTON MODE API FUNCTION***//
// Function   : getButtonMode
//
// Description: This function is redefinition of main getButtonMode function to match the types of API function arguments.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
...
//
// Return     : void
void getButtonMode(bool responseEnabled, bool apiEnabled, int* optionalArray)
{
  if (optionalArray[0] == 0)
  {
    getButtonMode(responseEnabled, apiEnabled);
  }
}
...
//
// Description: This function sets the button mode.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
//                                   Manual response is sent if it's set to false.
//               inputButtonMode : bool : The new the button mode.
//
// Return     : void
//*********************************//
void setButtonMode(bool responseEnabled, bool apiEnabled, int inputButtonMode)
{
  ...
  if (inputButtonMode == 1 || inputButtonMode == 2) // Check if inputButtonMode is within range
  {
    ...
    EEPROM.put(EEPROM_buttonMode, g_buttonMode);
    delay(EEPROM_WRITE_DELAY);

    if(!API_ENABLED){ g_buttonMode = BUTTON_MODE; }
    isValidFactor = true;
  }
  else
  {
    //Invalid inputButtonMode
    EEPROM.get(EEPROM_buttonMode, g_buttonMode);
    delay(10); 
    isValidFactor = false;
  }
...
// Function   : setButtonMode
//
// Description: This function is redefinition of main setButtonMode function to match the types of API function arguments.
//
// Parameters :  responseEnabled : bool : The response for serial printing is enabled if it's set to true.
//                                        The serial printing is ignored if it's set to false.
//               apiEnabled : bool : The API response is sent if it's set to true.
//                                   Manual response is sent if it's set to false.
//               inputButtonMode : int* : The array of one element which contains the new button mode.
//
// Return     : void
void setButtonMode(bool responseEnabled, bool apiEnabled, int* inputButtonMode)
{
  setButtonMode(responseEnabled, apiEnabled, inputButtonMode[0]);
}
...
//
// Description: This function handles the push button actions. Pressed individually, these change sensitivity and 
...
// Parameters :  void
//
// Return     : int : sip and puff raw action numbers
//*********************************//
...
void sipAndPuffHandler()
{
  ...
  { //Puff detected
  	...
  	else if(g_buttonMode == 2)
  	{
        joystickButtonPress(g_actionButton[0]);
  	}
  }
  ...
  if (g_joystickPressure > g_sipThreshold)
  {
    ...
    {
        joystickButtonPress(g_actionButton[1]);
    }	  
  }
  if (g_joystickPressure <= g_sipThreshold && g_joystickPressure >= g_puffThreshold && g_buttonMode == 2) 
  {  //Release buttons in analog trigger button mode
    joystickButtonRelease(g_actionButton[0]);
    joystickButtonRelease(g_actionButton[1]);
   }
}
...
void performButtonAction(byte outputAction)
{
	switch (outputAction)
    {
      ...
      case OUTPUT_BUTTON_ONE:
      case OUTPUT_BUTTON_TWO:
      case OUTPUT_BUTTON_THREE:
      case OUTPUT_BUTTON_FOUR:
      case OUTPUT_BUTTON_FIVE:
      case OUTPUT_BUTTON_SIX:
        {
          // Button Click: Perform joystick button click action
		      joystickButtonClick(outputAction);
          delay(5);
          break;
        }
...
//
// Description: This function performs joystick button click action.
//
// Parameters : byte : buttonNumber : The button number to be clicked.
//
// Return     : void
//****************************************//
void joystickButtonClick(byte buttonNumber)
{
  byte buttonIndex = buttonNumber - 1;
  ledClear();
  if (!g_lastButtonState[buttonIndex]) {
	  Joystick.pressButton(buttonIndex);
	  delay(JOYSTICK_BUTTON_DELAY);
	  Joystick.releaseButton(buttonIndex);
	  delay(50);
	  g_lastButtonState[buttonIndex] = 0;
	} 
}

//***JOYSTICK BUTTON PRESS FUNCTION***//
// Function   : joystickButtonPress
//
// Description: This function performs joystick button press action.
//
// Parameters : byte : buttonNumber : The button number to be pressed.
//
// Return     : void
//****************************************//
void joystickButtonPress(byte buttonNumber)
{
  byte buttonIndex = buttonNumber - 1;
  ledClear();
  Joystick.pressButton(buttonIndex);
  g_lastButtonState[buttonIndex] = 1;
}

//***JOYSTICK BUTTON RELEASE FUNCTION***//
// Function   : joystickButtonRelease
//
// Description: This function performs joystick button release action.
//
// Parameters : byte : buttonNumber : The button number to be released.
//
// Return     : void
//****************************************//
void joystickButtonRelease(byte buttonNumber)
{
  byte buttonIndex = buttonNumber - 1;
  ledClear();
  Joystick.releaseButton(buttonIndex);
